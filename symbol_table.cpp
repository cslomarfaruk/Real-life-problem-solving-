    ...
    while (getline(cin, s)) {
                ...
                if (!tmp.empty()) {
                    if (isKeyword(tmp)) {
                        ...
                        if (!currentType.empty()) {
                            if (declaredVariables.count(tmp)) {
                                ans = "Invalid\n";
                                cout << "Error: Variable '" << tmp << "' is already declared.\n";
                            } else {
                                declaredVariables.insert(tmp);
                                symbolTable[tmp] = currentType;
                            }
                        } else {
                            if (declaredVariables.find(tmp) == declaredVariables.end()) {
                                ans = "Invalid\n";
                                cout << "Error: Variable '" << tmp << "' is not declared before assignment.\n";
                            }
                        }
        ...
        if (!eq) {
            ...
            if ((result["identifier"].size() == 1 && result["special_symbol"].size() == 1) ||
                result["special_symbol"].size() == 2 && result["identifier"].size() == 1 &&
                star_loc < identifier_loc && (star_loc != -1 && identifier_loc != -1) ||
        ...
        cout << "\nSymbol Table:\n";
        cout << "-----------------------------------\n";
        cout << "SL.No\tVariable Name\tVariable Type\n";
        cout << "-----------------------------------\n";
        int index = 1;
        for (const auto &entry : symbolTable) {
            cout << index++ << "\t\t" << entry.first << "\t\t" << entry.second << "\n";
        }
        cout << "-----------------------------------\n";
    }
    return 0;
